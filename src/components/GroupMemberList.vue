...
<v-card class="mx-auto" outlined color="brown">
        ...
        <v-dialog v-model="addMemberDialog" max-width="800px" transition="dialog-bottom-transition" v-if="you.isAdmin" persistent>
            ...
            <v-card class="elevation-25" fluid fill-width>
                ...
                <v-card-actions class="form">
                    <div class="center">
                        <v-btn color="teal" @click="InviteUser()" elevation="8" class="btn">Invite</v-btn>
                    </div>
                </v-card-actions>
            </v-card>
    ...
    <v-flex>
        <v-sheet id="scrolling-techniques-6" class="overflow-y-auto" max-height="450" color="brown">
            <v-container>

                <v-card color="#385F73" dark class="users">
                    <v-card-title class="d-inline-flex">{{ you.username }}</v-card-title>
                    <v-card-text class="d-inline-flex"> {{ you.first_name }} {{ you.last_name }}
                ...
                <v-card color="#385F73" dark class="users" v-for="user in admins" :key="user.username">
                    ...
                    <v-card-text class="d-inline-flex"> {{ user.first_name }} {{ user.last_name }}
                        <v-row align="center" justify="end">
